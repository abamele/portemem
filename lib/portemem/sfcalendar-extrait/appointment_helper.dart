...
class AppointmentHelperT {
  ...
  static DateTime getMonthEndDate(DateTime date) {
    return DateTimeHelperT.getDateTimeValue(
        addDays(getNextMonthDate(date), -1));
  }
  ...
  static DateTime addDaysWithTime(
    ...
    final DateTime newDate =
        DateTimeHelperT.getDateTimeValue(addDays(date, days));
    return DateTime(
  ...
  static bool isCalendarAppointment(CalendarDataSourceT dataSource) {
    if (dataSource.appointments == null ||
        dataSource.appointments!.isEmpty ||
        dataSource.appointments![0] is CalendarAppointmentT) {
      return true;
    }
  ...
  static bool _isSpanned(CalendarAppointmentT appointment) {
  ...
  static bool canAddSpanIcon(List<DateTime> visibleDates,
      CalendarAppointmentT appointment, CalendarViewT view,
  ...
  /// appointment
  static String getSpanAppointmentText(CalendarAppointmentT appointment,
      DateTime date, SfLocalizationsT localization) {
    ...
    return 'Başlangıç Tarihi: ${DateFormat("dd/MM/yyyy HH:mm").format(appointment.startTime)}\n\nBitiş Tarihi:${DateFormat("dd/MM/yyyy HH:mm").format(appointment.endTime)}\n\nAçıklama: ${appointment.subject} (${localization.daySpanCountLabel} $currentDate / $totalDays)';
  }
  ...
  static List<CalendarAppointmentT> getSelectedDateAppointments(
  ...
  static TextStyle getAppointmentTextStyle(
      TextStyle appointmentTextStyle, CalendarViewT view) {
  ...
  static CalendarAppointmentT _copy(CalendarAppointmentT appointment) {
    final CalendarAppointmentT copyAppointment = CalendarAppointmentT(
        ...
        resourceIds: CalendarViewHelperT.cloneList(appointment.resourceIds),
  ...
  static bool _isAppointmentInVisibleDateRange(CalendarAppointmentT appointment,
  ...
  static void resetAppointmentView(
      List<AppointmentViewT> appointmentCollection) {
    for (int i = 0; i < appointmentCollection.length; i++) {
      final AppointmentViewT obj = appointmentCollection[i];
      obj.canReuse = true;
  ...
  static double timeToPosition(
      SfCalendarT calendar, DateTime date, double timeIntervalHeight) {
  ...
  static double getAppointmentHeightFromDuration(Duration? minimumDuration,
      SfCalendarT calendar, double timeIntervalHeight) {
  ...
  static bool _isIntersectingAppointmentInDayView(
      ...
      CalendarViewT view,
      CalendarAppointmentT currentApp,
      AppointmentViewT appView,
      CalendarAppointmentT appointment,
    ...
    final bool isTimelineMonth = view == CalendarViewT.timelineMonth;
  ...
  static bool _iterateAppointment(
      CalendarAppointmentT app, bool isTimeline, bool isAllDay) {
    if (isAllDay) {
  ...
  static AppointmentViewT _getAppointmentView(CalendarAppointmentT appointment,
      List<AppointmentViewT> appointmentCollection, int? resourceIndex) {
    AppointmentViewT? appointmentRenderer;
    for (int i = 0; i < appointmentCollection.length; i++) {
      final AppointmentViewT view = appointmentCollection[i];
    ...
    if (appointmentRenderer == null) {
      appointmentRenderer = AppointmentViewT();
  ...
  static void setAppointmentPositionAndMaxPosition(
    ...
    final List<CalendarAppointmentT> normalAppointments = visibleAppointments
    ...
    final Map<int, List<AppointmentViewT>> dict =
        <int, List<AppointmentViewT>>{};
    final List<AppointmentViewT> processedViews = <AppointmentViewT>[];
    int maxColsCount = 1;

    final int timeIntervalMinutes =
        CalendarViewHelperT.getTimeInterval(calendar.timeSlotViewSettings);
    for (int count = 0; count < normalAppointments.length; count++) {
      final CalendarAppointmentT currentAppointment = normalAppointments[count];
      if ((view == CalendarViewT.workWeek ||
      ...
      List<AppointmentViewT>? intersectingApps;
      final AppointmentViewT currentAppView = _getAppointmentView(
      ...
      for (int position = 0; position < maxColsCount; position++) {
        ...
        for (int j = 0; j < processedViews.length; j++) {
          final AppointmentViewT previousApp = processedViews[j];
          ...
          if (_isIntersectingAppointmentInDayView(
            ...
            if (intersectingApps == null) {
              ...
              if (intersectingApps == null) {
                intersectingApps = <AppointmentViewT>[];
  ...
  /// end date.
  static List<CalendarAppointmentT> getVisibleAppointments(
      DateTime visibleStartDate,
      DateTime visibleEndDate,
      List<CalendarAppointmentT> appointments,
    ...
    final List<CalendarAppointmentT> appointmentColl = <CalendarAppointmentT>[];
    final DateTime startDate = convertToStartTime(visibleStartDate);
    final DateTime endDate = convertToEndTime(visibleEndDate);
    final int count = appointments.length;

    for (int j = 0; j < count; j++) {
      final CalendarAppointmentT calendarAppointment = appointments[j];
      ...
      final List<CalendarAppointmentT> tempAppointments =
          <CalendarAppointmentT>[];
      ...
      for (int i = 0; i < appointmentLength; i++) {
        final CalendarAppointmentT appointment = tempAppointments[i];
        if (isAppointmentWithinVisibleDateRange(
            appointment, startDate, endDate)) {
          ...
          if (canCreateNewAppointment &&
              ...
              !isTimelineView) {
            for (int i = 0; i < 2; i++) {
              final CalendarAppointmentT spannedAppointment =
                  _copy(appointment);
              if (i == 0) {
            ...
            if (_isAppointmentInVisibleDateRange(
              ...
              for (int i = 0; i < 2; i++) {
                ...
                if (i == 0) {
                  spannedAppointment.actualStartTime =
                      appointment.actualStartTime;
                  final DateTime date =
                      DateTimeHelperT.getDateTimeValue(addDays(startDate, -1));
              ...
              for (int i = 0; i < 3; i++) {
                ...
                if (i == 0) {
                  ...
                  final DateTime date =
                      DateTimeHelperT.getDateTimeValue(addDays(endDate, 1));
                  spannedAppointment.actualStartTime =
  ...
  static CalendarAppointmentT _cloneRecurrenceAppointment(
      CalendarAppointmentT appointment,
      DateTime recursiveDate,
      String? calendarTimeZone) {
    final CalendarAppointmentT occurrenceAppointment = _copy(appointment);
    ...
    final int minutes =
        getDifference(appointment.actualStartTime, appointment.actualEndTime)
            .inMinutes;
    occurrenceAppointment.actualEndTime = DateTimeHelperT.getDateTimeValue(
        addDuration(
  ...
  /// collection.
  static List<CalendarAppointmentT> generateCalendarAppointments(
      CalendarDataSourceT? calendarData, String? calendarTimeZone,
      [List<dynamic>? appointments]) {
    final List<CalendarAppointmentT> calendarAppointmentCollection =
        <CalendarAppointmentT>[];
    ...
    if (dataSource.isNotEmpty && dataSource[0] is CalendarAppointmentT) {
      for (int i = 0; i < dataSource.length; i++) {
        final dynamic dataSourceItem = dataSource[i];
        late final CalendarAppointmentT item;
        if (dataSourceItem is CalendarAppointmentT) {
          item = dataSourceItem;
        }
  ...
  static CalendarAppointmentT _createAppointment(Object appointmentObject,
      CalendarDataSourceT calendarData, String? calendarTimeZone) {
    CalendarAppointmentT app;
    if (appointmentObject is AppointmentT) {
      app = CalendarAppointmentT(
  ...
  static void _updateTimeForInvalidEndTime(
      CalendarAppointmentT appointment, String? scheduleTimeZone) {
  ...
  static void _getRecurrenceAppointments(
    ...
    if (recursiveDates.isNotEmpty) {
      ...
      recDates = RecurrenceHelperT.getRecurrenceDateTimeCollection(
    ...
    for (int j = 0; j < recursiveDates.length; j++) {
      ...
      final CalendarAppointmentT occurrenceAppointment =
