...
class ResourceViewWidgetT extends StatefulWidget {
  /// Constructor to create the resource view widget to holds resource view on
  /// all timeline views.
  const ResourceViewWidgetT(
      this.resources,
  ...
  final List<CalendarResourceT>? resources;
...
class _ResourceViewWidgetState extends State<ResourceViewWidgetT> {
  @override
  Widget build(BuildContext context) {
    ...
    if (widget.resourceViewHeaderBuilder != null) {
      ...
      for (int i = 0; i < resourceLength; i++) {
        final CalendarResourceT currentResource = widget.resources![i];
        final Widget child = widget.resourceViewHeaderBuilder!(
            context,
            ResourceViewHeaderDetailsT(
                currentResource,
                Rect.fromLTWH(
    ...
    return _ResourceViewRenderObjectWidgetT(
      widget.resources,
...
class _ResourceViewRenderObjectWidgetT extends MultiChildRenderObjectWidget {
  _ResourceViewRenderObjectWidgetT(
      this.resources,
      this.resourceViewSettings,
      this.resourceItemHeight,
      this.cellBorderColor,
  ...
  @override
  _ResourceViewRenderObjectT createRenderObject(BuildContext context) {
    return _ResourceViewRenderObjectT(
        resources,
  ...
  void updateRenderObject(
      BuildContext context, _ResourceViewRenderObjectT renderObject) {
    renderObject
      ..resources = resources
...
class _ResourceViewRenderObjectT extends CustomCalendarRenderObjectT {
  _ResourceViewRenderObjectT(
      this._resources,
  ...
  List<CalendarResourceT>? _resources;

  List<CalendarResourceT>? get resources => _resources;

  set resources(List<CalendarResourceT>? value) {
    if (_resources == value) {
      return;
    }
  ...
  ResourceViewSettingsT _resourceViewSettings;

  ResourceViewSettingsT get resourceViewSettings => _resourceViewSettings;

  set resourceViewSettings(ResourceViewSettingsT value) {
  ...
  void _resourceViewHeader(Canvas canvas, Size size) {
    ...
    if (resourceViewSettings.showAvatar) {
      for (int i = 0; i < count; i++) {
        canvas.save();
        final CalendarResourceT resource = resources![i];
        _drawResourceBorder(
  ...
  void _drawResourceBackground(
      Canvas canvas, Size size, CalendarResourceT resource, double yPosition) {
    const double padding = 0.5;
  ...
  void _drawResourceBorder(CalendarResourceT resource, Canvas canvas, Size size,
  ...
  DecorationImagePainter? _getImagePainter(CalendarResourceT resource) {
  ...
  void _drawInnerCircle(CalendarResourceT resource, Canvas canvas, Size size,
  ...
  List<CustomPainterSemantics> _getSemanticsBuilder(Size size) {
    ...
    for (int j = 0; j < resources!.length; j++) {
      final CalendarResourceT resource = resources![j];
