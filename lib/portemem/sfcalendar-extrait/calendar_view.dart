...
import 'package:portemem/portemem/sfcalendar-extrait/all_day_appointment_layout.dart';
...
import 'package:portemem/portemem/sfcalendar-extrait/appointment_layout.dart';
...
import 'package:portemem/portemem/sfcalendar-extrait/day_view.dart';
...
import 'package:portemem/portemem/sfcalendar-extrait/month_view.dart';
...
import 'package:portemem/portemem/sfcalendar-extrait/time_line_view.dart';
...
@immutable
class CustomCalendarScrollViewT extends StatefulWidget {
  ...
  /// calendar widget.
  const CustomCalendarScrollViewT(
      this.calendar,
      this.view,
      this.width,
      this.height,
  ...
  final UpdateCalendarStateT updateCalendarState;

  /// Used to get the calendar state details.
  final UpdateCalendarStateT getCalendarState;
  ...
  final List<CalendarResourceT>? resourceCollection;
  ...
  CalendarDetailsT? getCalendarDetails(Offset position) {
    if (key == null) {
      return null;
    }
...
class _CustomCalendarScrollViewState extends State<CustomCalendarScrollViewT>
    with TickerProviderStateMixin {
  // three views to arrange the view in vertical/horizontal direction and handle the swiping
  late _CalendarViewT _currentView, _nextView, _previousView;

  // the three children which to be added into the layout
  final List<_CalendarViewT> _children = <_CalendarViewT>[];
  ...
  final UpdateCalendarStateDetailsT _updateCalendarStateDetails =
  UpdateCalendarStateDetailsT();

  /// Collection used to store the special regions and
  /// check the special regions manipulations.
  List<TimeRegionT>? _timeRegions;
  ...
  void initState() {
    ...
    _timeRegions = CalendarViewHelperT.cloneList(widget.specialRegions);

    super.initState();
  }

  @override
  void didUpdateWidget(CustomCalendarScrollViewT oldWidget) {
    if (oldWidget.controller != widget.controller) {
      ...
      if (!CalendarViewHelperT.isSameTimeSlot(oldWidget.controller.selectedDate,
    ...
    if (oldWidget.view != widget.view) {
      ...
      /// new widget tree.
      if (CalendarViewHelperT.isTimelineView(widget.view) !=
          CalendarViewHelperT.isTimelineView(oldWidget.view)) {
        _currentChildIndex = 1;
      }
    ...
    if ((widget.view == CalendarViewT.month ||
        widget.view == CalendarViewT.timelineMonth) &&
    ...
    if (oldWidget.calendar.timeSlotViewSettings !=
      ...
      CalendarViewHelperT.isTimelineView(widget.view);
      if (widget.view != CalendarViewT.month &&
  ...
  Widget build(BuildContext context) {
    if (!CalendarViewHelperT.isTimelineView(widget.view) &&
        widget.view != CalendarViewT.month) {
    ...
    final bool isDayView = CalendarViewHelperT.isDayView(
        widget.view,
        widget.calendar.timeSlotViewSettings.numberOfDaysInView,
        widget.calendar.timeSlotViewSettings.nonWorkingDays,
    ...
    final bool isNeedDragAndDrop = widget.calendar.allowDragAndDrop &&
        widget.view != CalendarViewT.schedule &&
    ...
    final double timeLabelWidth = CalendarViewHelperT.getTimeLabelWidth(
        widget.calendar.timeSlotViewSettings.timeRulerSize, widget.view);
    final bool isResourceEnabled = CalendarViewHelperT.isResourceEnabled(
    ...
    final bool isMonthView = widget.view == CalendarViewT.month ||
        widget.view == CalendarViewT.timelineMonth;
              ...
              child: IgnorePointer(
                  child: RepaintBoundary(
                      child: _DraggingAppointmentWidget(
                          ...
                          widget.isMobilePlatform,
                          AppointmentHelperT.getAppointmentTextStyle(
  ...
  void _handleAppointmentDragStart(
    ...
    } else if (widget.view == CalendarViewT.month) {
    ...
    CalendarResourceT? selectedResource;
    ...
    yPosition <= 0 && widget.view != CalendarViewT.month && !isTimelineView
        ? null
  ...
  AppointmentViewT? _getDragAppointment(
  ...
  void _updateAppointmentDragUpdateCallback(
    ...
    CalendarResourceT? selectedResource, previousResource;
    ...
    final int timeInterval = CalendarViewHelperT.getTimeInterval(
        widget.calendar.timeSlotViewSettings);
    ...
    if (CalendarViewHelperT.isDraggingAppointmentHasDisabledCell(
        _getTimeRegions(),
        _getBlackoutDates(),
        updateStartTime,
        updatedEndTime,
        ...
        (widget.view == CalendarViewT.month &&
            !CalendarViewHelperT.isCurrentMonthDate(
                widget.calendar.monthViewSettings.numberOfWeeksInView,
  ...
  void _handleLongPressEnd(
    ...
    final CalendarAppointmentT? appointment =
    ...
    if (!isTimelineView && widget.view != CalendarViewT.month) {
    ...
    updateStartTime = AppointmentHelperT.convertTimeToAppointmentTimeZone(
        updateStartTime, widget.calendar.timeZone, appointment.startTimeZone);
    updatedEndTime = AppointmentHelperT.convertTimeToAppointmentTimeZone(
    ...
    CalendarAppointmentT? parentAppointment;
    if ((appointment.recurrenceRule != null &&
      ...
      i++) {
        final CalendarAppointmentT app =
      ...
      i < _updateCalendarStateDetails.appointments.length;
      i++) {
        final CalendarAppointmentT calendarApp =
        _updateCalendarStateDetails.appointments[i];
      ...
      if (!CalendarViewHelperT.isDateInDateCollection(
      ...
      if (appointment.recurrenceId != null &&
        ...
        widget.calendar.dataSource!.notifyListeners(
            CalendarDataSourceActionT.remove, <dynamic>[appointment.data]);
      } else {
        widget.calendar.dataSource!.appointments!
            .remove(parentAppointment.data);
        widget.calendar.dataSource!.notifyListeners(
            CalendarDataSourceActionT.remove, <dynamic>[parentAppointment.data]);
        ...
        widget.calendar.dataSource!.notifyListeners(
            CalendarDataSourceActionT.add, <dynamic>[newParentAppointment]);
      }
    } else {
      widget.calendar.dataSource!.appointments!.remove(appointment.data);
      widget.calendar.dataSource!.notifyListeners(
    ...
    widget.calendar.dataSource!.notifyListeners(
        CalendarDataSourceActionT.add, <dynamic>[newAppointment]);
  ...
  _CalendarViewState? _getCurrentViewByVisibleDates() {
    ...
    for (int i = 0; i < _children.length; i++) {
      final _CalendarViewT currentView = _children[i];
  ...
  void _updateVisibleDates() {
    widget.getCalendarState(_updateCalendarStateDetails);
    final List<int>? nonWorkingDays = (widget.view == CalendarViewT.workWeek ||
        widget.view == CalendarViewT.timelineWorkWeek)
        ? widget.calendar.timeSlotViewSettings.nonWorkingDays
        : null;
    ...
    final DateTime prevDate = DateTimeHelperT.getPreviousViewStartDate(
        widget.view,
        widget.calendar.monthViewSettings.numberOfWeeksInView,
        currentDate,
        visibleDatesCount,
        nonWorkingDays);
    final DateTime nextDate = DateTimeHelperT.getNextViewStartDate(
        widget.view,
        widget.calendar.monthViewSettings.numberOfWeeksInView,
        currentDate,
        visibleDatesCount,
    ...
    _nextViewVisibleDates = getVisibleDates(widget.isRTL ? prevDate : nextDate,
        ...
        .cast();
    if (widget.view == CalendarViewT.timelineMonth) {
      _visibleDates = DateTimeHelperT.getCurrentMonthDates(_visibleDates);
      _previousViewVisibleDates =
          DateTimeHelperT.getCurrentMonthDates(_previousViewVisibleDates);
      _nextViewVisibleDates =
          DateTimeHelperT.getCurrentMonthDates(_nextViewVisibleDates);
    }
  ...
  void _updateNextViewVisibleDates() {
    ...
    if (widget.isRTL) {
      currentViewDate = DateTimeHelperT.getPreviousViewStartDate(
          widget.view,
          widget.calendar.monthViewSettings.numberOfWeeksInView,
          currentViewDate,
          visibleDatesCount,
      ...
      currentViewDate = DateTimeHelperT.getNextViewStartDate(
          widget.view,
          widget.calendar.monthViewSettings.numberOfWeeksInView,
          currentViewDate,
          visibleDatesCount,
    ...
    if (widget.view == CalendarViewT.timelineMonth) {
      dates = DateTimeHelperT.getCurrentMonthDates(dates);
    }
  ...
  void _getCalendarViewStateDetails(UpdateCalendarStateDetailsT details) {
    widget.getCalendarState(_updateCalendarStateDetails);
  ...
  void _updateCalendarViewStateDetails(UpdateCalendarStateDetailsT details) {
    _updateCalendarStateDetails.selectedDate = details.selectedDate;
    widget.updateCalendarState(_updateCalendarStateDetails);
  }

  CalendarTimeRegion _getCalendarTimeRegionFromTimeRegion(TimeRegionT region) {
  ...
  List<CalendarTimeRegion> _getRegions(List<DateTime> visibleDates) {
    ...
    AppointmentHelperT.convertToStartTime(visibleStartDate);
    final DateTime endDate = AppointmentHelperT.convertToEndTime(visibleEndDate);
    for (int j = 0; j < _timeRegions!.length; j++) {
      final TimeRegionT timeRegion = _timeRegions![j];
      ...
      region.actualEndTime = AppointmentHelperT.convertTimeToAppointmentTimeZone(
      ...
      if (region.recurrenceRule == null || region.recurrenceRule == '') {
        if (AppointmentHelperT.isDateRangeWithinVisibleDateRange(
  ...
  CalendarTimeRegion cloneRecurrenceRegion(CalendarTimeRegion region,
    ...
    final int minutes = AppointmentHelperT.getDifference(
        region.actualStartTime, region.actualEndTime)
        .inMinutes;
    final DateTime actualEndTime = DateTimeHelperT.getDateTimeValue(
        addDuration(recursiveDate, Duration(minutes: minutes)));
    final DateTime startDate =
    AppointmentHelperT.convertTimeToAppointmentTimeZone(
        recursiveDate, calendarTimeZone, region.timeZone);

    final DateTime endDate = AppointmentHelperT.convertTimeToAppointmentTimeZone(
        actualEndTime, calendarTimeZone, region.timeZone);

    final TimeRegionT occurrenceTimeRegion =
  ...
  List<Widget> _addViews() {
    if (_children.isEmpty) {
      _previousView = _CalendarViewT(
        ...
        _dragDetails,
            (UpdateCalendarStateDetailsT details) {
  ...
  _CalendarViewT _updateViews(_CalendarViewT view,
    ...
    else if (!CalendarViewHelperT.isCollectionEqual(
  ...
  bool _isTimeRegionsEquals(
      List<TimeRegionT>? regions1, List<TimeRegionT>? regions2) {
  ...
  void _selectResourceProgrammatically() {
    ...
    for (int i = 0; i < _children.length; i++) {
      ...
      if (CalendarViewHelperT.isResourceEnabled(
          widget.calendar.dataSource, widget.view)) {
  ...
  void _updateSelectedResourceIndex() {
    for (int i = 0; i < _children.length; i++) {
      ...
      if (selectedResourceIndex != -1) {
        ...
        final int newIndex = CalendarViewHelperT.getResourceIndex(
  ...
  void _moveToNextViewWithAnimation() {
    ...
    if (widget.calendar.monthViewSettings.navigationDirection ==
        MonthNavigationDirectionT.vertical &&
        widget.view == CalendarViewT.month) {
  ...
  void _moveToPreviousWebViewWithAnimation({bool isScrollToEnd = false}) {
    ...
    if (isTimelineView) {
      _positionTimelineView(isScrolledToEnd: isScrollToEnd);
    } else if (!isTimelineView && widget.view != CalendarViewT.month) {
    ...
    /// expanded, to collapse the all day panel in day, week and work week view,
    /// we have added this condition and called the method.
    if (widget.view != CalendarViewT.month && !isTimelineView) {
      _updateAllDayPanel();
    }
  ...
  DateTime _updateSelectedDateForRightArrow(_CalendarViewT currentView,
    ...
    if (!CalendarViewHelperT.isTimelineView(widget.view)) {
      ...
      selectedDate = AppointmentHelperT.addDaysWithTime(selectedDate!, 1,
      ...
      if (widget.view == CalendarViewT.month &&
          !CalendarViewHelperT.isCurrentMonthDate(
              ...
              selectedDate)) {
        _moveToNextViewWithAnimation();
      } else if (widget.view == CalendarViewT.workWeek) {
        for (int i = 0;
        i <
      ...
      final double xPosition = widget.view == CalendarViewT.timelineMonth
          ? 0
          : AppointmentHelperT.timeToPosition(widget.calendar, selectedDate!,
      ...
      if (widget.view == CalendarViewT.timelineDay &&
          selectedDate
      ...
      if (widget.view == CalendarViewT.timelineWorkWeek) {
        for (int i = 0;
        i <
            DateTime.daysPerWeek -
                widget.calendar.timeSlotViewSettings.nonWorkingDays.length;
  ...
  DateTime _updateSelectedDateForLeftArrow(_CalendarViewT currentView,
      _CalendarViewState currentViewState, DateTime? selectedDate) {
    if (!CalendarViewHelperT.isTimelineView(widget.view)) {
  ...
  DateTime? _updateSelectedDateForUpArrow(
    ...
    if (widget.view == CalendarViewT.month) {
      ...
      selectedDate = AppointmentHelperT.addDaysWithTime(
          selectedDate,
          -DateTime.daysPerWeek,
          selectedDate.hour,
          selectedDate.minute,
      ...
      /// previous month date.
      if (!CalendarViewHelperT.isCurrentMonthDate(
          widget.calendar.monthViewSettings.numberOfWeeksInView,
          widget.calendar.monthViewSettings.showTrailingAndLeadingDates,
          currentView.visibleDates[currentView.visibleDates.length ~/ 2].month,
          selectedDate)) {
        selectedDate = AppointmentHelperT.getMonthStartDate(
            currentViewState._selectionPainter!.selectedDate ??
        ...
        if (CalendarViewHelperT.isDateInDateCollection(
          ...
          do {
            selectedDate = AppointmentHelperT.addDaysWithTime(selectedDate!, 1,
                selectedDate.hour, selectedDate.minute, selectedDate.second);
          } while (CalendarViewHelperT.isDateInDateCollection(
              currentView.blackoutDates, selectedDate));
        }
      }

      return selectedDate;
    } else if (!CalendarViewHelperT.isTimelineView(widget.view)) {
      final double yPosition = AppointmentHelperT.timeToPosition(
  ...
  DateTime? _updateSelectedDateForDownArrow(
    ...
    if (widget.view == CalendarViewT.month) {
      ...
      if (!CalendarViewHelperT.isCurrentMonthDate(
          ...
          currentView.visibleDates[currentView.visibleDates.length ~/ 2].month,
          selectedDate)) {
        selectedDate = AppointmentHelperT.getMonthEndDate(
            currentViewState._selectionPainter!.selectedDate ??
      ...
      final double viewHeaderHeight = CalendarViewHelperT.getViewHeaderHeight(
          widget.calendar.viewHeaderHeight, widget.view);
      final double yPosition = AppointmentHelperT.timeToPosition(
          widget.calendar, selectedDate!, currentViewState._timeIntervalHeight);
  ...
  DateTime? _updateSelectedDate(
    ...
    if (event.logicalKey == LogicalKeyboardKey.arrowRight) {
      ...
      do {
        ...
        if (widget.controller.view != CalendarViewT.month &&
            !CalendarViewHelperT.isTimelineView(widget.calendar.view)) {
          double yPosition = AppointmentHelperT.timeToPosition(widget.calendar,
              selectedDate!, currentViewState._timeIntervalHeight);
          if (yPosition < 1 &&
              !_isSelectedDateEnabled(selectedDate, resourceIndex, true)) {
            yPosition = AppointmentHelperT.timeToPosition(
                widget.calendar,
                currentViewState._selectionPainter!.selectedDate!,
                currentViewState._timeIntervalHeight);
  ...
  bool _isSelectedDateEnabled(DateTime date, int resourceIndex,
    ...
    if (isMonthView &&
        CalendarViewHelperT.isDateInDateCollection(_getBlackoutDates(), date)) {
      return false;
    } else if (!isMonthView) {
      ...
      for (int i = 0; i < regions.length; i++) {
        ...
        if (region.enablePointerInteraction ||
            ...
            CalendarViewHelperT.isSameTimeSlot(region.actualEndTime, date)) {
          continue;
        }
  ...
  KeyEventResult _updatePageUpAndDown(RawKeyEvent event,
    ...
    if (widget.controller.view != CalendarViewT.day &&
        widget.controller.view != CalendarViewT.week &&
        widget.controller.view != CalendarViewT.workWeek &&
        !isResourceEnabled) {
    ...
    if (event.logicalKey == LogicalKeyboardKey.pageUp ||
      ...
      double viewHeaderHeight = CalendarViewHelperT.getViewHeaderHeight(
      ...
      final double timeRulerSize = CalendarViewHelperT.getTimeLabelWidth(
  ...
  KeyEventResult _updateAppointmentSelection(
      ...
      bool isResourceEnabled,
      AppointmentViewT? currentSelectedAppointment,
      AppointmentViewT? currentAllDayAppointment) {
    if (widget.controller.view == CalendarViewT.schedule) {
      return KeyEventResult.ignored;
    }

    AppointmentViewT? selectedAppointment;
    bool isAllDay = currentAllDayAppointment != null;
    final List<AppointmentViewT> appointmentCollection = currentVisibleViewState
        ._appointmentLayout
        .getAppointmentViewCollection();
    final List<AppointmentViewT> allDayAppointmentCollection =
        _updateCalendarStateDetails.allDayAppointmentViewCollection;
    final List<AppointmentViewT> tempAppColl =
        ...
        if (currentSelectedAppointment != null && selectedAppointment == null) {
          ...
          if (currentVisibleViewState._selectionPainter!.selectedDate != null &&
            ...
            for (int i = 0; i < appointmentCollection.length; i++) {
              if (AppointmentHelperT.getDifference(
  ...
  KeyEventResult _updateAppointmentSelectionOnView(
      ...
      bool isResourceEnabled,
      bool isForward) {
    final DateTime visibleStartDate = AppointmentHelperT.convertToStartTime(
        currentVisibleViewState.widget.visibleDates[0]);
    final DateTime visibleEndDate = AppointmentHelperT.convertToEndTime(
    ...
    if (selectedAppointment != null &&
        AppointmentHelperT.isAppointmentWithinVisibleDateRange(
      ...
      if (widget.controller.view != CalendarViewT.month) {
        ...
        CalendarViewHelperT.isTimelineView(widget.controller.view!);
        double viewHeaderHeight = CalendarViewHelperT.getViewHeaderHeight(
            widget.calendar.viewHeaderHeight, widget.controller.view!);
      ...
      } else if (widget.controller.view == CalendarViewT.month) {
  ...
  KeyEventResult _onKeyDown(FocusNode node, RawKeyEvent event) {
    ...
    if (event.isControlPressed && widget.view != CalendarViewT.schedule) {
      final bool canMoveToNextView = DateTimeHelperT.canMoveToNextView(
          widget.view,
          widget.calendar.monthViewSettings.numberOfWeeksInView,
          widget.calendar.minDate,
          widget.calendar.maxDate,
          ...
          widget.calendar.timeSlotViewSettings.nonWorkingDays,
          widget.isRTL);
      final bool canMoveToPreviousView = DateTimeHelperT.canMoveToPreviousView(
          widget.view,
          widget.calendar.monthViewSettings.numberOfWeeksInView,
          widget.calendar.minDate,
          widget.calendar.maxDate,
    ...
    _CalendarViewState currentVisibleViewState;
    _CalendarViewT currentVisibleView;
    final bool isResourcesEnabled = CalendarViewHelperT.isResourceEnabled(
        widget.calendar.dataSource, widget.view);
    if (_currentChildIndex == 0) {
    ...
    if (event.logicalKey == LogicalKeyboardKey.enter &&
        CalendarViewHelperT.shouldRaiseCalendarTapCallback(
            widget.calendar.onTap)) {
      final AppointmentViewT? selectedAppointment = currentVisibleViewState
          ._allDaySelectionNotifier.value?.appointmentView ??
          currentVisibleViewState._selectionPainter!.appointmentView;
      final List<CalendarAppointmentT>? selectedAppointments =
      widget.controller.view == CalendarViewT.month &&
          selectedAppointment == null
          ...
          _updateCalendarStateDetails.selectedDate)
          : selectedAppointment != null
          ? <CalendarAppointmentT>[selectedAppointment.appointment!]
          : null;
      final CalendarElementT tappedElement =
      _updateCalendarStateDetails.selectedDate != null
      ...
      CalendarViewHelperT.raiseCalendarTapCallback(
          widget.calendar,
          tappedElement == CalendarElementT.appointment
    ...
    if ((currentVisibleViewState._selectionPainter!.selectedDate != null &&
      ...
      final DateTime? selectedAppointmentDate = currentAllDayAppointment != null
          ? AppointmentHelperT.convertToStartTime(
  ...
  void _onHorizontalStart(
    ...
    switch (widget.calendar.viewNavigationMode) {
      case ViewNavigationModeT.none:
        return;
      case ViewNavigationModeT.snap:
        widget.removePicker();
        if (widget.calendar.monthViewSettings.navigationDirection ==
            MonthNavigationDirectionT.horizontal ||
            widget.view != CalendarViewT.month) {
    ...
    switch (widget.calendar.viewNavigationMode) {
      ...
      case ViewNavigationModeT.snap:
        ...
        if (widget.calendar.monthViewSettings.navigationDirection ==
          ...
          if (difference < 0 &&
              !DateTimeHelperT.canMoveToNextView(
                  widget.view,
                  widget.calendar.monthViewSettings.numberOfWeeksInView,
                  widget.calendar.minDate,
                  widget.calendar.maxDate,
            ...
            return;
              ...
              !DateTimeHelperT.canMoveToPreviousView(
                  widget.view,
                  widget.calendar.monthViewSettings.numberOfWeeksInView,
                  widget.calendar.minDate,
                  widget.calendar.maxDate,
  ...
  void _updateAppointmentPainter() {
    for (int i = 0; i < _children.length; i++) {
      final _CalendarViewT view = _children[i];
      final GlobalKey<_CalendarViewState> viewKey =
      // ignore: avoid_as
      ...
      if (widget.view == CalendarViewT.month &&
        ...
        if (view.visibleDates == _currentViewVisibleDates) {
          ...
          if (!CalendarViewHelperT.isEmptyList(viewKey
        ...
        if (view.visibleDates == _currentViewVisibleDates) {
          ...
          if (!CalendarViewHelperT.isEmptyList(
              appointmentLayout.visibleAppointments.value)) {
...
@immutable
class _CalendarViewT extends StatefulWidget {
  const _CalendarViewT(
      this.calendar,
      this.view,
      this.visibleDates,
      this.width,
...
class _CalendarViewState extends State<_CalendarViewT>
    with TickerProviderStateMixin {
  ...
  late AppointmentLayoutT _appointmentLayout;
  ...
  late TimelineViewHeaderViewT _timelineViewHeader;
  _SelectionPainterT? _selectionPainter;
  ...
  AppointmentViewT? _hoveringAppointmentView;
  ...
  late MonthViewWidgetT _monthView;
  ...
  void initState() {
    ...
    _timeIntervalHeight = _getTimeIntervalHeight(
        ...
        widget.isMobilePlatform);
    if (widget.view != CalendarViewT.month) {
      _horizontalLinesCount = CalendarViewHelperT.getHorizontalLinesCount(
  ...
  @override
  void didUpdateWidget(_CalendarViewT oldWidget) {
    final bool isTimelineView = CalendarViewHelperT.isTimelineView(widget.view);
    if (widget.view != CalendarViewT.month) {
      if (!isTimelineView) {
    ...
    if ((oldWidget.view == CalendarViewT.month ||
        oldWidget.view == CalendarViewT.schedule ||
    ...
    widget.getCalendarState(_updateCalendarStateDetails);
    if (!CalendarViewHelperT.isTimelineView(oldWidget.view) &&
        _updateCalendarStateDetails.selectedDate != null &&
        CalendarViewHelperT.isResourceEnabled(
            widget.calendar.dataSource, widget.view) &&
  ...
  void _updateAllDayHeight(bool isCurrentView) {
    if (widget.view != CalendarViewT.day &&
        widget.view != CalendarViewT.week &&
        widget.view != CalendarViewT.workWeek) {
      return;
    }
  ...
  void _timelineViewHoveringUpdate() {
    if (!CalendarViewHelperT.isTimelineView(widget.view) && mounted) {
      return;
    }
  ...
  double _getScrollPositionForCurrentDate(DateTime date) {
    ...
    if (!CalendarViewHelperT.isTimelineView(widget.view)) {
      timeToPosition = AppointmentHelperT.timeToPosition(
      ...
      for (int i = 0; i < visibleDatesCount; i++) {
        ...
        if (widget.view == CalendarViewT.timelineMonth) {
          ...
          timeToPosition = (_getSingleViewWidthForTimeLineView(this) * i) +
              AppointmentHelperT.timeToPosition(
                  widget.calendar, date, _timeIntervalHeight);
        }
  ...
  void _retainScrolledDateTime() {
    ...
    final double time = ((CalendarViewHelperT.getTimeInterval(
        widget.calendar.timeSlotViewSettings) /
        60) *
        columnIndex) +
  ...
  double _getPositionFromDate(DateTime date) {
    ...
    if (!isTimelineView) {
      ...
      final double scrollViewContentHeight =
          CalendarViewHelperT.getHorizontalLinesCount(
              widget.calendar.timeSlotViewSettings, widget.view) *
              _timeIntervalHeight;
  ...
  void _updateTimeSlotView(_CalendarViewT oldWidget) {
    _animationController ??= AnimationController(
        duration: const Duration(milliseconds: 200), vsync: this);
    _heightAnimation ??=
    CurveTween(curve: Curves.easeIn).animate(_animationController!)
    ...
    if (!CalendarViewHelperT.isDayView(
        widget.view,
        widget.calendar.timeSlotViewSettings.numberOfDaysInView,
        widget.calendar.timeSlotViewSettings.nonWorkingDays,
        widget.calendar.monthViewSettings.numberOfWeeksInView) &&
  ...
  void _updateHorizontalLineCount(_CalendarViewT oldWidget) {
    if (widget.calendar.timeSlotViewSettings.startHour !=
        ...
        CalendarViewHelperT.getTimeInterval(
            widget.calendar.timeSlotViewSettings) !=
            CalendarViewHelperT.getTimeInterval(
                oldWidget.calendar.timeSlotViewSettings) ||
        oldWidget.view == CalendarViewT.month ||
        oldWidget.view == CalendarViewT.timelineMonth ||
        oldWidget.view != CalendarViewT.timelineMonth &&
            widget.view == CalendarViewT.timelineMonth) {
      _horizontalLinesCount = CalendarViewHelperT.getHorizontalLinesCount(
          widget.calendar.timeSlotViewSettings, widget.view);
  ...
  void _updateTimelineViews(_CalendarViewT oldWidget) {
    _timelineRulerController ??= ScrollController()
      ..addListener(_timeRulerListener);
  ...
  void _getPainterProperties(UpdateCalendarStateDetailsT details) {
    widget.getCalendarState(_updateCalendarStateDetails);
    details.allDayAppointmentViewCollection =
        _updateCalendarStateDetails.allDayAppointmentViewCollection;
    details.currentViewVisibleDates =
  ...
  Widget _addAllDayAppointmentPanel(
      SfCalendarThemeDataT calendarTheme, bool isCurrentView) {
    final bool isDayView = CalendarViewHelperT.isDayView(
        widget.view,
        widget.calendar.timeSlotViewSettings.numberOfDaysInView,
        widget.calendar.timeSlotViewSettings.nonWorkingDays,
    ...
    final double timeLabelWidth = CalendarViewHelperT.getTimeLabelWidth(
        widget.calendar.timeSlotViewSettings.timeRulerSize, widget.view);
    double topPosition = CalendarViewHelperT.getViewHeaderHeight(
        widget.calendar.viewHeaderHeight, widget.view);
    if (isDayView) {
  ...
  Widget _getAllDayLayout(double timeLabelWidth, double panelHeight,
    ...
    final bool isDayView = CalendarViewHelperT.isDayView(
        ...
        widget.calendar.timeSlotViewSettings.nonWorkingDays,
        widget.calendar.monthViewSettings.numberOfWeeksInView);
    final Widget allDayLayout = AllDayAppointmentLayoutT(
        widget.calendar,
        widget.view,
        widget.visibleDates,
        widget.visibleDates ==
  ...
  Widget _addAppointmentPainter(double width, double height,
    ...
    widget.visibleDates ==
        ...
        ? _updateCalendarStateDetails.visibleAppointments
        : null;
    _appointmentLayout = AppointmentLayoutT(
      widget.calendar,
      widget.view,
      widget.visibleDates,
      ValueNotifier<List<CalendarAppointmentT>?>(visibleAppointments),
      _timeIntervalHeight,
      widget.calendarTheme,
      _isRTL,
  ...
  void _onVerticalStart(DragStartDetails details) {
    ...
    if (CalendarViewHelperT.shouldRaiseAppointmentResizeStartCallback(
        widget.calendar.onAppointmentResizeStart)) {
      CalendarViewHelperT.raiseAppointmentResizeStartCallback(
          widget.calendar,
  ...
  void _onVerticalEnd(DragEndDetails details) {
    ...
    if (AppointmentHelperT.canAddSpanIcon(
    ...
    updatedStartTime = AppointmentHelperT.convertTimeToAppointmentTimeZone(
    ...
    if (CalendarViewHelperT.isDraggingAppointmentHasDisabledCell(
        ...
        -1,
        widget.resourceCollection)) {
      if (CalendarViewHelperT.shouldRaiseAppointmentResizeEndCallback(
          widget.calendar.onAppointmentResizeEnd)) {
        CalendarViewHelperT.raiseAppointmentResizeEndCallback(
            widget.calendar,
            appointment.data,
            null,
            appointment.exactStartTime,
    ...
    if (CalendarViewHelperT.shouldRaiseAppointmentResizeEndCallback(
        widget.calendar.onAppointmentResizeEnd)) {
      CalendarViewHelperT.raiseAppointmentResizeEndCallback(widget.calendar,
  ...
  void _onHorizontalUpdate(DragUpdateDetails details) {
    ...
    if (isTimelineView) {
      ...
      if (!widget.calendar.monthViewSettings.showTrailingAndLeadingDates &&
        ...
        final int startIndex = DateTimeHelperT.getVisibleDateIndex(
            widget.visibleDates,
            AppointmentHelperT.getMonthStartDate(currentMonthDate));
        final int endIndex = DateTimeHelperT.getVisibleDateIndex(
            widget.visibleDates,
            AppointmentHelperT.getMonthEndDate(currentMonthDate));
    ...
    if (CalendarViewHelperT.shouldRaiseAppointmentResizeUpdateCallback(
        widget.calendar.onAppointmentResizeUpdate)) {
      CalendarViewHelperT.raiseAppointmentResizeUpdateCallback(
          widget.calendar,
          _getCalendarAppointmentToObject(
              _resizingDetails.value.appointmentView!.appointment,
              widget.calendar),
  ...
  void _onHorizontalEnd(DragEndDetails details) {
    ...
    final bool isMonthView = widget.view == CalendarViewT.timelineMonth ||
        widget.view == CalendarViewT.month;
      ...
      case CalendarViewT.timelineMonth:
        {
          ...
          double minimumTimeIntervalSize = timeIntervalSize /
              (widget.view == CalendarViewT.timelineMonth ? 2 : 4);
          if (minimumTimeIntervalSize < 20) {
            minimumTimeIntervalSize = 20;
          }
          if (isForwardResize) {
      ...
      children: <Widget>[
        Positioned(
          ...
          child: Container(
            ...
            child: RepaintBoundary(
              child: CustomPaint(
                painter: _ViewHeaderViewPainterT(
                    widget.visibleDates,
                    widget.view,
                    widget.calendar.viewHeaderStyle,
                    widget.calendar.timeSlotViewSettings,
                    CalendarViewHelperT.getTimeLabelWidth(
                        widget.calendar.timeSlotViewSettings.timeRulerSize,
                        widget.view),
                    CalendarViewHelperT.getViewHeaderHeight(
                        widget.calendar.viewHeaderHeight, widget.view),
  ...
  Widget _getResizeShadowView() {
    ...
    final bool isAllDayPanel = !isVerticalResize &&
        (!isTimelineView && widget.view != CalendarViewT.month);
    final double timeLabelWidth = CalendarViewHelperT.getTimeLabelWidth(
        widget.calendar.timeSlotViewSettings.timeRulerSize, widget.view);
    final double weekNumberPanelWidth =
    CalendarViewHelperT.getWeekNumberPanelWidth(
  ...
  void _updateProgrammaticSelectedResourceIndex() {
    if (_updateCalendarStateDetails.selectedDate != null &&
        _selectedResourceIndex == -1) {
      final bool isTimelineMonth = widget.view == CalendarViewT.timelineMonth;
      if ((isTimelineMonth &&
  ...
  Widget _addTimelineView(double width, double height, String locale) {
    ...
    if (isResourceEnabled) {
      ...
      resourceItemHeight = CalendarViewHelperT.getResourceItemHeight(
          resourceViewSize,
  ...
  CalendarDetailsT? _getCalendarViewDetails(Offset position) {
    switch (widget.view) {
      case CalendarViewT.day:
      case CalendarViewT.week:
      case CalendarViewT.workWeek:
  ...
  CalendarDetailsT? _getDetailsForMonth(Offset position) {
    ...
    if (widget.calendar.monthViewSettings.appointmentDisplayMode ==
        MonthAppointmentDisplayModeT.appointment) {
      appointmentView =
          _appointmentLayout.getAppointmentViewOnPoint(xPosition, yPosition);
    ...
    appointmentView == null || isMoreTapped
        ...
        : <dynamic>[
      CalendarViewHelperT.getAppointmentDetail(
          appointmentView.appointment!, widget.calendar.dataSource)
    ];
    final CalendarElementT selectedElement = appointmentView == null
        ? CalendarElementT.calendarCell
        : isMoreTapped
        ? CalendarElementT.moreAppointmentRegion
        : CalendarElementT.appointment;
  ...
  AppointmentViewT? _handleTouchOnMonthView(
    ...
    if (yDetails < viewHeaderHeight) {
      ...
      CalendarViewHelperT.shouldRaiseCalendarSelectionChangedCallback(
      ...
      if (canRaiseLongPress || canRaiseTap || canRaiseSelectionChanged) {
        ...
        final CalendarElementT selectedElement = appointmentView == null
            ...
            : CalendarElementT.appointment;
        if (canRaiseTap) {
          CalendarViewHelperT.raiseCalendarTapCallback(widget.calendar,
              selectedDate, selectedAppointments, selectedElement, null);
        } else if (canRaiseLongPress) {
          CalendarViewHelperT.raiseCalendarLongPressCallback(widget.calendar,
              selectedDate, selectedAppointments, selectedElement, null);
        }
  ...
  void _handleMonthCellTapNavigation(DateTime date) {
    ...
    widget.controller.view = CalendarViewT.day;
  ...
  CalendarDetailsT? _getDetailsForTimeline(Offset position) {
    ...
    final double timeLabelHeight = CalendarViewHelperT.getTimeLabelWidth(
        widget.calendar.timeSlotViewSettings.timeRulerSize, widget.view);
    ...
    CalendarResourceT? calendarResource;
    ...
    if (appointmentView == null) {
      ...
      return CalendarDetailsT(
          null, getDate, CalendarElementT.calendarCell, calendarResource);
    } else {
      /// Return calendar details while the [getCalendarDetailsAtOffset]
      /// position placed on appointment in timeline views.
      return CalendarDetailsT(<dynamic>[
        CalendarViewHelperT.getAppointmentDetail(
            appointmentView.appointment!, widget.calendar.dataSource)
      ], getDate, CalendarElementT.appointment, calendarResource);
    }
  }

  /// Handles the tap and long press related functions for timeline view.
  AppointmentViewT? _handleTouchOnTimeline(
      TapUpDetails? tapDetails, LongPressStartDetails? longPressDetails) {
    widget.removePicker();
    final DateTime? previousSelectedDate = _selectionPainter!.selectedDate;
    double xDetails = 0, yDetails = 0;
    ...
    if (yDetails < viewHeaderHeight) {
      ...
      if (canRaiseLongPress || canRaiseTap || canRaiseSelectionChanged) {
        ...
        final int timeInterval = CalendarViewHelperT.getTimeInterval(
            widget.calendar.timeSlotViewSettings);
        if (appointmentView == null) {
          if (!CalendarViewHelperT.isDateTimeWithInDateTimeRange(
              widget.calendar.minDate,
              widget.calendar.maxDate,
              selectedDate,
              timeInterval) ||
  ...
  void _updateAllDaySelection(AppointmentViewT? view, DateTime? date) {
    if (_allDaySelectionNotifier.value != null &&
  ...
  CalendarDetailsT? _getDetailsForDay(Offset position) {
    final double xDetails = position.dx;
    final double yDetails = position.dy;
    ...
    if (yDetails < viewHeaderHeight) {
      ...
      final AppointmentViewT? appointmentView = _getAllDayAppointmentOnPoint(
          _updateCalendarStateDetails.allDayAppointmentViewCollection,
          xDetails,
          yPosition);
      ...
      List<CalendarAppointmentT>? moreRegionAppointments;
      ...
      if (isTappedOnCount && selectedDate != null) {
        final int currentSelectedIndex = DateTimeHelperT.getVisibleDateIndex(
            widget.visibleDates, selectedDate);
        if (currentSelectedIndex != -1) {
          moreRegionAppointments = <CalendarAppointmentT>[];
          for (int i = 0;
          i <
              _updateCalendarStateDetails
                  .allDayAppointmentViewCollection.length;
          i++) {
            final AppointmentViewT currentView =
      ...
      if (appointmentView == null &&
        ...
        if (currentSelectedIndex != -1) {
          moreRegionAppointments = <CalendarAppointmentT>[];
          final List<AppointmentViewT> selectedIndexAppointment =
          <AppointmentViewT>[];
          for (int i = 0;
          i <
              _updateCalendarStateDetails
      ...
      if (appointmentView != null &&
        ...
        return CalendarDetailsT(
            appointmentDetails, null, CalendarElementT.appointment, null);
        ...
        return CalendarDetailsT(
            null, selectedDate, CalendarElementT.allDayPanel, null);
      }
    ...
    if (appointmentView == null) {
      ...
      return CalendarDetailsT(
          null, selectedDate, CalendarElementT.calendarCell, null);
    } else {
      final List<dynamic> appointmentDetails = <dynamic>[
        CalendarViewHelperT.getAppointmentDetail(
            appointmentView.appointment!, widget.calendar.dataSource)
  ...
  /// work week views.
  AppointmentViewT? _handleTouchOnDayView(
      TapUpDetails? tapDetails, LongPressStartDetails? longPressDetails) {
    widget.removePicker();
    final DateTime? previousSelectedDate = _selectionPainter!.selectedDate;
    final int timeInterval = CalendarViewHelperT.getTimeInterval(
    ...
    widget.getCalendarState(_updateCalendarStateDetails);
    AppointmentViewT? selectedAppointmentView;
    dynamic selectedAppointment;
    List<dynamic>? selectedAppointments;
    CalendarElementT targetElement = CalendarElementT.viewHeader;
    DateTime? selectedDate = _updateCalendarStateDetails.selectedDate;
    final double timeLabelWidth = CalendarViewHelperT.getTimeLabelWidth(
        widget.calendar.timeSlotViewSettings.timeRulerSize, widget.view);
    ...
    if (yDetails < viewHeaderHeight) {
      ...
      if (appointmentView == null) {
        targetElement = CalendarElementT.allDayPanel;
        if (isTappedCallback) {
          selectedDate =
      ...
      if (appointmentView != null &&
        ...
        targetElement = CalendarElementT.appointment;
      ...
      if (appointmentView == null) {
        ...
        targetElement = CalendarElementT.calendarCell;
      } else {
        if (selectedDate != null) {
          selectedDate = null;
          _selectionPainter!.selectedDate = selectedDate;
    ...
    widget.updateCalendarState(_updateCalendarStateDetails);
    final bool canRaiseTap = CalendarViewHelperT.shouldRaiseCalendarTapCallback(
        widget.calendar.onTap) &&
        isTappedCallback;
    final bool canRaiseLongPress =
        CalendarViewHelperT.shouldRaiseCalendarLongPressCallback(
    ...
    if (canRaiseLongPress || canRaiseTap || canRaiseSelectionChanged) {
      ...
      if (_selectionPainter!.selectedDate != null &&
        ...
        /// Restrict the callback, while selected region as disabled
        /// [TimeRegion].
        if (targetElement == CalendarElementT.calendarCell &&
            !_isEnabledRegion(
  ...
  bool _isAutoTimeIntervalHeight(SfCalendarT calendar, bool isTimelineView) {
    if (isTimelineView) {
  ...
  double _getTimeIntervalWidth(double timeIntervalHeight, CalendarViewT view,
    ...
    if (view == CalendarViewT.timelineMonth &&
        !CalendarViewHelperT.isMobileLayoutUI(width, isMobilePlatform)) {
      return 160;
    }
  ...
  double _getTimeIntervalHeight(
    ...
    final bool isTimelineView = CalendarViewHelperT.isTimelineView(view);
    final bool isDayView = CalendarViewHelperT.isDayView(
        view,
    ...
    double viewHeaderHeight =
    CalendarViewHelperT.getViewHeaderHeight(calendar.viewHeaderHeight, view);
  ...
  /// scrollable width
  bool _isValidWidth(double screenWidth, SfCalendarT calendar,
  ...
  void _handleOnTapForViewHeader(TapUpDetails details, double width) {
    ...
    CalendarViewHelperT.raiseCalendarTapCallback(
        widget.calendar, tappedDate, null, CalendarElementT.viewHeader, null);
  }
  ...
  void _handleViewHeaderTapNavigation(DateTime date) {
    if (!widget.allowViewNavigation ||
        widget.view == CalendarViewT.day ||
        widget.view == CalendarViewT.timelineDay ||
        widget.view == CalendarViewT.month) {
      return;
    }
    ...
    if (!isDateWithInDateRange(
        widget.calendar.minDate, widget.calendar.maxDate, date) ||
        (widget.controller.view == CalendarViewT.timelineMonth &&
            CalendarViewHelperT.isDateInDateCollection(
                widget.blackoutDates, date))) {
      return;
    }

    if (widget.view == CalendarViewT.week ||
        widget.view == CalendarViewT.workWeek) {
      widget.controller.view = CalendarViewT.day;
    } else {
      widget.controller.view = CalendarViewT.timelineDay;
    }
  ...
  DateTime? _getTappedViewHeaderDate(Offset localPosition, double width) {
    int index = 0;
    final double timeLabelViewWidth = CalendarViewHelperT.getTimeLabelWidth(
        widget.calendar.timeSlotViewSettings.timeRulerSize, widget.view);
    ...
    if (!CalendarViewHelperT.isTimelineView(widget.view)) {
      ...
      /// RTL.
      if (_isRTL && widget.view != CalendarViewT.month) {
        index = visibleDatesLength - index - 1;
      } else if (_isRTL && widget.view == CalendarViewT.month) {
  ...
  void _updateMouseCursorForAppointment(AppointmentViewT? appointmentView,
    ...
    if (isAllDayPanel ||
        (widget.view == CalendarViewT.month || isTimelineViews)) {
      final bool isMonthView = widget.view == CalendarViewT.month ||
          widget.view == CalendarViewT.timelineMonth;
      final DateTime viewStartDate =
      AppointmentHelperT.convertToStartTime(widget.visibleDates[0]);
      final DateTime viewEndDate = AppointmentHelperT.convertToEndTime(
          widget.visibleDates[widget.visibleDates.length - 1]);
      ...
      final DateTime appointmentEndTime = appointmentView.appointment!.isAllDay
          ? AppointmentHelperT.convertToEndTime(
          appointmentView.appointment!.actualEndTime)
          : appointmentView.appointment!.actualEndTime;
  ...
  void _updatePointerHover(Offset globalPosition) {
    ...
    double xPosition;
    double yPosition;
    final bool isTimelineViews = CalendarViewHelperT.isTimelineView(widget.view);
    if (widget.view != CalendarViewT.month && !isTimelineViews) {
      /// In LTR, remove the time ruler width value from the
      ...
      if (localPosition.dy < viewHeaderHeight) {
        if (isDayView) {
          if ((_isRTL && localPosition.dx < widget.width - timeLabelWidth) ||
            ...
            final AppointmentViewT? appointment = _getAllDayAppointmentOnPoint(
                _updateCalendarStateDetails.allDayAppointmentViewCollection,
                localPosition.dx,
                localPosition.dy);
      ...
      final AppointmentViewT? appointment =
      ...
      if (widget.calendar.showWeekNumber && widget.view == CalendarViewT.month) {
        final double weekNumberPanelWidth =
        CalendarViewHelperT.getWeekNumberPanelWidth(
            widget.calendar.showWeekNumber,
            widget.width,
    ...
    if (!isResourceEnabled) {
      if ((widget.view == CalendarViewT.month &&
          ...
          _viewHeaderNotifier.value == null) ||
          (widget.view != CalendarViewT.month &&
              CalendarViewHelperT.isSameTimeSlot(_hoveringDate, hoverDate) &&
              _viewHeaderNotifier.value == null)) {
        return;
      }
    }
  ...
  AppointmentViewT? _getAllDayAppointmentOnPoint(
      List<AppointmentViewT>? appointmentCollection, double x, double y) {
    ...
    AppointmentViewT? selectedAppointmentView;
    for (int i = 0; i < appointmentCollection.length; i++) {
      final AppointmentViewT appointmentView = appointmentCollection[i];
  ...
  _SelectionPainterT _addSelectionView([double? resourceItemHeight]) {
    AppointmentViewT? appointmentView;
    if (_selectionPainter?.appointmentView != null) {
      appointmentView = _selectionPainter!.appointmentView;
    }

    _selectionPainter = _SelectionPainterT(
      widget.calendar,
      widget.view,
      widget.visibleDates,
      _updateCalendarStateDetails.selectedDate,
  ...
  Widget _getTimelineViewHeader(double width, double height, String locale) {
    _timelineViewHeader = TimelineViewHeaderViewT(
        widget.visibleDates,
...
class _ViewHeaderViewPainterT extends CustomPainter {
  _ViewHeaderViewPainterT(
      this.visibleDates,
      this.view,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    double width = view == CalendarViewT.month
  ...
  void _addViewHeaderForTimeSlotViews(
    ...
    for (int i = 0; i < visibleDatesLength; i++) {
      ...
      if (!isDayView &&
        ...
        DateTimeHelperT.getWeekNumberOfYear(currentDate).toString();
        final TextStyle weekNumberTextStyle =
            weekNumberStyle.textStyle ?? calendarTheme.weekNumberTextStyle!;
  ...
  @override
  bool shouldRepaint(_ViewHeaderViewPainterT oldDelegate) {
    final _ViewHeaderViewPainterT oldWidget = oldDelegate;
  ...
  @override
  bool shouldRebuildSemantics(_ViewHeaderViewPainterT oldDelegate) {
    final _ViewHeaderViewPainterT oldWidget = oldDelegate;
    return oldWidget.visibleDates != visibleDates;
  }
  ...
  List<CustomPainterSemantics> _getSemanticsForDayHeader(Size size) {
    ...
    for (int i = 0; i < visibleDates.length; i++) {
      ...
      if (showWeekNumber &&
        ...
        final int weekNumber = DateTimeHelperT.getWeekNumberOfYear(visibleDate);
        semanticsBuilder.add(CustomPainterSemantics(
...
class _SelectionPainterT extends CustomPainter {
  _SelectionPainterT(
      this.calendar,
      this.view,
      this.visibleDates,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    CalendarViewHelperT.getTimeInterval(calendar.timeSlotViewSettings);
    if (selectedDate != null &&
        ((isMonthView &&
            !isDateWithInDateRange(
    ...
    final bool isTimeline = CalendarViewHelperT.isTimelineView(view);
    if (view != CalendarViewT.month && !isTimeline) {
      width -= timeLabelWidth;
    }

    final bool isResourceEnabled = isTimeline &&
        CalendarViewHelperT.isResourceEnabled(calendar.dataSource, view);
  ...
  void _drawDaySelection(
    ...
    if (isSameDate(visibleDates[0], selectedDate)) {
      ...
      _yPosition = AppointmentHelperT.timeToPosition(
  ...
  @override
  bool shouldRepaint(_SelectionPainterT oldDelegate) {
    final _SelectionPainterT oldWidget = oldDelegate;
...
class _TimeRulerViewT extends CustomPainter {
  _TimeRulerViewT(
  ...
  void _drawTimeLabels(Canvas canvas, Size size, DateTime date, double hour,
    ...
    const int padding = 5;
    final int timeInterval =
    CalendarViewHelperT.getTimeInterval(timeSlotViewSettings);
  ...
  @override
  bool shouldRepaint(_TimeRulerViewT oldDelegate) {
    final _TimeRulerViewT oldWidget = oldDelegate;
...
class _MultiChildContainerRenderObject extends RenderStack {
  ...
  SemanticsBuilderCallback get _semanticsBuilder {
    ...
    child = childAfter(child) as RenderRepaintBoundary?) {
      if (child.child is! CustomCalendarRenderObjectT) {
        continue;
      }

      final CustomCalendarRenderObjectT appointmentRenderObject =
      // ignore: avoid_as
      child.child! as CustomCalendarRenderObjectT;
...
class _CurrentTimeIndicator extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final double minuteHeight = timeIntervalSize /
        CalendarViewHelperT.getTimeInterval(timeSlotViewSettings);
    final double currentTimePosition = CalendarViewHelperT.getTimeToPosition(
...
DateTime? _timeFromPosition(
    DateTime date,
    TimeSlotViewSettingsT timeSlotViewSettings,
    double positionY,
    _CalendarViewState? currentState,
    double timeIntervalHeight,
    bool isTimelineView) {
  ...
  final double singleIntervalHeightForAnHour =
      (60 / CalendarViewHelperT.getTimeInterval(timeSlotViewSettings)) *
          timeIntervalHeight;
...
class _ResizingAppointmentPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    view == CalendarViewT.month || resizingDetails.value.isAllDayPanel
        ? 0
  ...
  void _drawTimeIndicator(
    ...
    if (view == CalendarViewT.month || view == CalendarViewT.timelineMonth) {
      return;
    }
  ...
  void _addRecurrenceIcon(Rect rect, Canvas canvas, int? textPadding,
    ...
    final TextSpan icon = AppointmentHelperT.getRecurrenceIcon(
...
dynamic _getCalendarAppointmentToObject(
    CalendarAppointmentT? calendarAppointment, SfCalendarT calendar) {
  if (calendarAppointment == null) {
    return null;
  }

  final AppointmentT appointment =
  calendarAppointment.convertToCalendarAppointment();

  if (calendarAppointment.data is AppointmentT) {
    return appointment;
  }
...
class _DraggingAppointmentWidget extends StatefulWidget {
  ...
  final CalendarViewT calendarView;
...
class _DraggingAppointmentState extends State<_DraggingAppointmentWidget> {
  AppointmentViewT? _draggingAppointmentView;
...
class _DraggingAppointmentRenderObject extends RenderBox
  ...
  DragAndDropSettingsT _dragAndDropSettings;

  DragAndDropSettingsT get dragAndDropSettings => _dragAndDropSettings;

  set dragAndDropSettings(DragAndDropSettingsT value) {
  ...
  CalendarViewT _calendarView;

  CalendarViewT get calendarView => _calendarView;

  set calendarView(CalendarViewT value) {
  ...
  @override
  void paint(PaintingContext context, Offset offset) {
    final bool isTimelineView = CalendarViewHelperT.isTimelineView(calendarView);
    if (child == null) {
  ...
  void _drawTimeIndicator(Canvas canvas, bool isTimelineView, Size size) {
    if (calendarView == CalendarViewT.month ||
        calendarView == CalendarViewT.timelineMonth) {
      return;
    }
