...
import 'package:portemem/portemem/sfcalendar-extrait/month_appointment_helper.dart';
...
class AppointmentLayoutT extends StatefulWidget {
  ...
  final ValueNotifier<List<CalendarAppointmentT>?> visibleAppointments;

  /// Return the appointment view based on x and y position.
  AppointmentViewT? getAppointmentViewOnPoint(double x, double y) {
    // ignore: avoid_as
  ...
  List<AppointmentViewT> getAppointmentViewCollection() {
    // ignore: avoid_as
    final GlobalKey appointmentLayoutKey = key! as GlobalKey;
    final _AppointmentLayoutState state =
    // ignore: avoid_as
...
class _AppointmentLayoutState extends State<AppointmentLayoutT> {
  /// It holds the appointment views for the visible appointments.
  final List<AppointmentViewT> _appointmentCollection = <AppointmentViewT>[];

  /// It holds the appointment list based on its visible index value.
  Map<int, List<AppointmentViewT>> _indexAppointments =
  <int, List<AppointmentViewT>>{};
  ...
  void initState() {
    ...
    _weekNumberPanelWidth = CalendarViewHelperT.getWeekNumberPanelWidth(
  ...
  @override
  void didUpdateWidget(AppointmentLayoutT oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    if (_children.isEmpty && widget.calendar.appointmentBuilder != null) {
      ...
      for (int i = 0; i < _appointmentCollection.length; i++) {
        final AppointmentViewT appointmentView = _appointmentCollection[i];
      ...
      if (_monthAppointmentCountViews.isNotEmpty) {
        ...
        for (int i = 0; i < keys.length; i++) {
          ...
          final List<CalendarAppointmentT> moreAppointments =
          <CalendarAppointmentT>[];
          final List<AppointmentViewT> moreAppointmentViews =
          ...
          for (int j = 0; j < moreAppointmentViews.length; j++) {
            final AppointmentViewT currentAppointment = moreAppointmentViews[j];
  ...
  AppointmentViewT? _getAppointmentViewOnPoint(double x, double y) {
    ...
    if (selectedAppointmentView == null &&
      ...
      for (int i = 0; i < keys.length; i++) {
        ...
        if (rect != null &&
            ...
            rect.top <= y &&
            rect.bottom >= y) {
          selectedAppointmentView = AppointmentViewT()..appointmentRect = rect;
          break;
        }
      }
    }
  ...
  List<CalendarAppointmentT> _getValidAppointments(
      List<CalendarAppointmentT> visibleAppointments) {
    ...
    final List<CalendarAppointmentT> appointments = <CalendarAppointmentT>[];
    ...
    for (int i = 0; i < visibleAppointments.length; i++) {
      final CalendarAppointmentT appointment = visibleAppointments[i];
  ...
  void _updateAppointmentDetails() {
    ...
    _indexAppointments = <int, List<AppointmentViewT>>{};
    widget.updateCalendarState(_updateCalendarStateDetails);
    AppointmentHelperT.resetAppointmentView(_appointmentCollection);
    _children.clear();
  ...
  void _updateMonthAppointmentDetails(
    ...
    AppointmentHelperT.convertToEndTime(widget.visibleDates[count - 1]);
    ...
    if (!showTrailingLeadingDates) {
      ...
      visibleStartDate = AppointmentHelperT.convertToStartTime(
          AppointmentHelperT.getMonthStartDate(currentMonthDate));
      visibleEndDate = AppointmentHelperT.convertToEndTime(
          AppointmentHelperT.getMonthEndDate(currentMonthDate));
      visibleStartIndex =
          DateTimeHelperT.getIndex(widget.visibleDates, visibleStartDate);
      visibleEndIndex =
          DateTimeHelperT.getIndex(widget.visibleDates, visibleEndDate);
    }

    MonthAppointmentHelperT.updateAppointmentDetails(
        visibleAppointments,
        _appointmentCollection,
        widget.visibleDates,
        _indexAppointments,
    ...
    // in month view
    final double cellEndPadding = CalendarViewHelperT.getCellEndPadding(
  ...
  void _updateDayAppointmentDetails(
    ...
    AppointmentHelperT.setAppointmentPositionAndMaxPosition(
        _appointmentCollection,
        widget.calendar,
        widget.view,
        visibleAppointments,
    ...
    for (int i = 0; i < _appointmentCollection.length; i++) {
      ...
      final CalendarAppointmentT appointment = appointmentView.appointment!;
      int column = -1;
      ...
      Duration difference = AppointmentHelperT.getDifference(
  ...
  void _updateTimelineMonthAppointmentDetails(
    ...
    if (isResourceEnabled) {
      for (int i = 0; i < widget.calendar.dataSource!.resources!.length; i++) {
        ...
        final List<CalendarAppointmentT> appointmentForEachResource =
        visibleAppointments
            .where((CalendarAppointmentT app) =>
    ...
    for (int i = 0; i < _appointmentCollection.length; i++) {
      ...
      int index =
      DateTimeHelperT.getVisibleDateIndex(widget.visibleDates, startTime);
      ...
      AppointmentHelperT.getDifference(startTime, endTime);
  ...
  void _updateTimelineAppointmentDetails(
    ...
    for (int i = 0; i < _appointmentCollection.length; i++) {
      ...
      if (widget.calendar.timeSlotViewSettings.minimumAppointmentDuration !=
        ...
        AppointmentHelperT.getAppointmentHeightFromDuration(
            widget.calendar.timeSlotViewSettings.minimumAppointmentDuration,
            widget.calendar,
            widget.timeIntervalHeight);
...
class _AppointmentRenderWidget extends MultiChildRenderObjectWidget {
  ...
  final SfLocalizationsT localizations;
  final List<CalendarAppointmentT>? visibleAppointments;
  final List<AppointmentViewT> appointmentCollection;
  final Map<int, List<AppointmentViewT>> indexAppointments;
  final Map<int, RRect> monthAppointmentCountViews;

  @override
  _AppointmentRenderObjectT createRenderObject(BuildContext context) {
    return _AppointmentRenderObjectT(
        calendar,
        view,
        visibleDates,
        visibleAppointments,
  ...
  void updateRenderObject(
      BuildContext context, _AppointmentRenderObjectT renderObject) {
    renderObject
      ..calendar = calendar
      ..view = view
...
class _AppointmentRenderObjectT extends CustomCalendarRenderObjectT {
  _AppointmentRenderObjectT(
      this._calendar,
      this._view,
      this._visibleDates,
  ...
  set visibleAppointments(List<CalendarAppointmentT>? value) {
    if (CalendarViewHelperT.isCollectionEqual(_visibleAppointments, value)) {
      return;
    }
  ...
  SfLocalizationsT _localizations;

  SfLocalizationsT get localizations => _localizations;

  set localizations(SfLocalizationsT value) {
  ...
  List<CalendarResourceT>? get resourceCollection => _resourceCollection;

  set resourceCollection(List<CalendarResourceT>? value) {
  ...
  CalendarViewT get view => _view;

  set view(CalendarViewT value) {
    if (_view == value) {
      return;
    }
  ...
  SfCalendarT _calendar;

  SfCalendarT get calendar => _calendar;

  set calendar(SfCalendarT value) {
    if (_calendar == value) {
      return;
    }
  ...
  List<AppointmentViewT> appointmentCollection = <AppointmentViewT>[];
  Map<int, List<AppointmentViewT>> indexAppointments =
  <int, List<AppointmentViewT>>{};
    ...
    for (int i = 0; i < appointmentCollection.length; i++) {
      ...
      semanticsBuilder.add(CustomPainterSemantics(
        ...
        properties: SemanticsProperties(
          label: CalendarViewHelperT.getAppointmentSemanticsText(
  ...
  void _drawMonthAppointment(Canvas canvas, Size size, Paint paint) {
    ...
    switch (calendar.monthViewSettings.appointmentDisplayMode) {
      case MonthAppointmentDisplayModeT.none:
        return;
      case MonthAppointmentDisplayModeT.appointment:
        _drawMonthAppointmentView(canvas, size, cellWidth, cellHeight, paint);
        break;
      case MonthAppointmentDisplayModeT.indicator:
  ...
  void _drawMonthAppointmentView(Canvas canvas, Size size, double cellWidth,
    ...
    DateTime visibleStartDate =
    AppointmentHelperT.convertToStartTime(visibleDates[0]);
    DateTime visibleEndDate =
    AppointmentHelperT.convertToEndTime(visibleDates[count - 1]);
    final bool showTrailingLeadingDates =
    CalendarViewHelperT.isLeadingAndTrailingDatesVisible(
    ...
    for (int i = 0; i < appointmentCollection.length; i++) {
      ...
      if (appointmentView.position < maximumDisplayCount ||
          (appointmentView.position == maximumDisplayCount &&
              appointmentView.maxPositions == maximumDisplayCount)) {
        final CalendarAppointmentT appointment = appointmentView.appointment!;
        final bool canAddSpanIcon = AppointmentHelperT.canAddSpanIcon(
        ...
        TextStyle style = AppointmentHelperT.getAppointmentTextStyle(
        ...
        if (canAddSpanIcon) {
          final int appStartIndex = MonthAppointmentHelperT.getDateIndex(
              appointment.exactStartTime, visibleDates);
          final int appEndIndex = MonthAppointmentHelperT.getDateIndex(
  ...
  void _drawForwardSpanIconForMonth(
      ...
      Paint paint) {
    final TextSpan icon =
    AppointmentHelperT.getSpanIcon(style.color!, textSize, !isRTL);
    _textPainter.text = icon;
    _textPainter.layout(maxWidth: rect.width > 0 ? rect.width : 0);

    final double yPosition =
    AppointmentHelperT.getYPositionForSpanIcon(icon, _textPainter, rect);
  ...
  void _drawMonthAppointmentIndicator(
    ...
    for (int i = 0; i < visibleDatesCount; i++) {
      ...
      final List<CalendarAppointmentT> appointmentLists =
  ...
  List<CalendarAppointmentT> _getSpecificDateVisibleAppointment(DateTime? date) {
    ...
    final DateTime startDate = AppointmentHelperT.convertToStartTime(date);
    final DateTime endDate = AppointmentHelperT.convertToEndTime(date);

    for (int j = 0; j < visibleAppointments!.length; j++) {
      final CalendarAppointmentT appointment = visibleAppointments![j];
  ...
  void _drawDayAppointments(Canvas canvas, Size size, Paint paint) {
    ...
    CalendarViewHelperT.isMobileLayoutUI(size.width, isMobilePlatform);
    for (int i = 0; i < appointmentCollection.length; i++) {
      final AppointmentViewT appointmentView = appointmentCollection[i];
      if (appointmentView.canReuse ||
          appointmentView.appointmentRect == null ||
      ...
      AppointmentHelperT.canAddSpanIcon(visibleDates, appointment, view);
      ...
      if (canAddSpanIcon) {
        ...
        } else if (!CalendarViewHelperT.isSameTimeSlot(
            appointment.exactStartTime, appointment.actualStartTime) &&
            CalendarViewHelperT.isSameTimeSlot(
                appointment.exactEndTime, appointment.actualEndTime)) {
  ...
  void _addBackwardSpanIconForDay(Canvas canvas, RRect rect, Size size,
    ...
    final TextSpan icon = AppointmentHelperT.getSpanIcon(
  ...
  void _drawTimelineAppointments(Canvas canvas, Size size, Paint paint) {
    ...
    for (int i = 0; i < appointmentCollection.length; i++) {
      ...
      if (canAddSpanIcon) {
        ...
        if (AppointmentHelperT.canAddForwardSpanIcon(
            appStartTime, appEndTime, viewStartDate, viewEndDate)) {
          forwardSpanIconSize = iconSize;
        } else if (AppointmentHelperT.canAddBackwardSpanIcon(
            appStartTime, appEndTime, viewStartDate, viewEndDate)) {
          backwardSpanIconSize = iconSize;
        } else {
          forwardSpanIconSize = iconSize;
  ...
  String _getTimelineAppointmentText(
      CalendarAppointmentT appointment, bool canAddSpanIcon) {
    if (view != CalendarViewT.timelineDay || !canAddSpanIcon) {
      return appointment.subject;
    }

    return AppointmentHelperT.getSpanAppointmentText(
