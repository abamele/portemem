...
class _TimelineWidgetState extends State<TimelineWidgetT> {
  ...
  @override
  void didUpdateWidget(TimelineWidgetT oldWidget) {
    ...
    if (_children.isEmpty &&
      ...
      for (int i = 0; i < count; i++) {
        ...
        final Widget child = widget.timeRegionBuilder!(
            context,
            TimeRegionDetailsT(view.region.data,
  ...
  void _updateSpecialRegionDetails() {
    ...
    final double minuteHeight = widget.timeIntervalWidth /
        CalendarViewHelperT.getTimeInterval(widget.timeSlotViewSettings);
    final DateTime startDate =
    AppointmentHelperT.convertToStartTime(widget.visibleDates[0]);
    final DateTime endDate = AppointmentHelperT.convertToEndTime(
    ...
    for (int i = 0; i < widget.specialRegion!.length; i++) {
      ...
      if (CalendarViewHelperT.isSameTimeSlot(regionStartTime, regionEndTime)) {
        continue;
      }
      ...
      int startIndex = DateTimeHelperT.getVisibleDateIndex(
          widget.visibleDates, regionStartTime);
      int endIndex = DateTimeHelperT.getVisibleDateIndex(
          widget.visibleDates, regionEndTime);

      double startXPosition = CalendarViewHelperT.getTimeToPosition(
          Duration(
      ...
      double endXPosition = CalendarViewHelperT.getTimeToPosition(
      ...
      if (isResourceEnabled &&
        ...
        for (int i = 0; i < region.resourceIds!.length; i++) {
          final int index = CalendarViewHelperT.getResourceIndex(
...
class _TimelineRenderWidgetT extends MultiChildRenderObjectWidget {
  _TimelineRenderWidgetT(
      this.horizontalLinesCountPerView,
      this.visibleDates,
      this.timeSlotViewSettings,
      this.timeIntervalWidth,
  ...
  @override
  _TimelineRenderObjectT createRenderObject(BuildContext context) {
    return _TimelineRenderObjectT(
  ...
  void updateRenderObject(
      BuildContext context, _TimelineRenderObjectT renderObject) {
    renderObject
...
class _TimelineRenderObjectT extends CustomCalendarRenderObjectT {
  _TimelineRenderObjectT(
  ...
  TimeSlotViewSettingsT _timeSlotViewSettings;

  TimeSlotViewSettingsT get timeSlotViewSettings => _timeSlotViewSettings;

  set timeSlotViewSettings(TimeSlotViewSettingsT value) {
  ...
  List<CalendarResourceT>? resourceCollection;
  ...
  set specialRegion(List<CalendarTimeRegion>? value) {
    if (CalendarViewHelperT.isCollectionEqual(_specialRegion, value)) {
      return;
    }
  ...
  set minDate(DateTime value) {
    if (CalendarViewHelperT.isSameTimeSlot(_minDate, value)) {
      return;
    }
  ...
  set maxDate(DateTime value) {
    if (CalendarViewHelperT.isSameTimeSlot(_maxDate, value)) {
      return;
    }
  ...
  set blackoutDates(List<DateTime>? value) {
    if (CalendarViewHelperT.isDateCollectionEqual(_blackoutDates, value)) {
      return;
    }
  ...
  void _drawDisabledDate(
    ...
    final int dateIndex =
    DateTimeHelperT.getVisibleDateIndex(visibleDates, disabledDate);
    double leftPosition = 0;
    const double topPosition = 0;

    final double xPosition = isTimelineMonth
        ? 0
        : CalendarViewHelperT.getTimeToPosition(
  ...
  String _getAccessibilityText(DateTime date, [CalendarResourceT? resource]) {
    String dateText;
...
class TimelineViewHeaderViewT extends CustomPainter {
  /// Constructor to create the view header view to holds header cell for
  /// timeline views.
  TimelineViewHeaderViewT(
      this.visibleDates,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < visibleDatesLength; i++) {
      ...
      CalendarViewHelperT.isDateInDateCollection(blackoutDates, currentDate);
  ...
  @override
  bool shouldRepaint(TimelineViewHeaderViewT oldDelegate) {
    final TimelineViewHeaderViewT oldWidget = oldDelegate;
  ...
  String _getAccessibilityText(DateTime date) {
    ...
    if (CalendarViewHelperT.isDateInDateCollection(blackoutDates, date)) {
  ...
  @override
  bool shouldRebuildSemantics(TimelineViewHeaderViewT oldDelegate) {
    final TimelineViewHeaderViewT oldWidget = oldDelegate;
    return oldWidget.visibleDates != visibleDates;
  }
}
