...
class MonthAppointmentHelperT {
  ...
  static void _createAppointmentInfoForSpannedAppointment(
    ...
    if (appointmentView.startIndex ~/ DateTime.daysPerWeek !=
      ...
      final AppointmentViewT appointmentView1 =
      _createAppointmentView(appointmentCollection);
  ...
  static void _setAppointmentPosition(
      List<AppointmentViewT> appointmentViewCollection,
      AppointmentViewT appointmentView,
      int viewIndex) {
    for (int j = 0; j < appointmentViewCollection.length; j++) {
      ...
      final AppointmentViewT prevAppointmentView = appointmentViewCollection[j];
  ...
  static bool _isInterceptAppointments(
      AppointmentViewT appointmentView1, AppointmentViewT appointmentView2) {
  ...
  static int _orderAppointmentViewBySpanned(
    ...
    final CalendarAppointmentT appointment1 = appointmentView1.appointment!;
    final CalendarAppointmentT appointment2 = appointmentView2.appointment!;

    /// Calculate the both appointment start time based on isAllDay property.
    final DateTime startTime1 = appointment1.isAllDay
        ? AppointmentHelperT.convertToStartTime(appointment1.exactStartTime)
        : appointment1.exactStartTime;
    final DateTime startTime2 = appointment2.isAllDay
        ? AppointmentHelperT.convertToStartTime(appointment2.exactStartTime)
    ...
    final DateTime endTime1 = appointment1.isAllDay
        ? AppointmentHelperT.convertToEndTime(appointment1.exactEndTime)
        : appointment1.exactEndTime;
    final DateTime endTime2 = appointment2.isAllDay
        ? AppointmentHelperT.convertToEndTime(appointment2.exactEndTime)
    ...
    return (AppointmentHelperT.getDifference(endTime2, startTime2)
        .inMinutes
        .abs())
        .compareTo(AppointmentHelperT.getDifference(endTime1, startTime1)
        .inMinutes
        .abs());
  }
  ...
  static void _updateAppointmentPosition(
    ...
    for (int j = 0; j < appointmentCollection.length; j++) {
      final AppointmentViewT appointmentView = appointmentCollection[j];
      ...
      i++) {
        ...
        final List<AppointmentViewT>? existingAppointments =
        indexAppointments[i];
        if (existingAppointments != null) {
          existingAppointments.add(appointmentView);
          indexAppointments[i] = existingAppointments;
        } else {
          indexAppointments[i] = <AppointmentViewT>[appointmentView];
        }
      }
    }
  }
  ...
  static int getDateIndex(DateTime date, List<DateTime> visibleDates) {
    ...
    DateTimeHelperT.getDateTimeValue(addDays(dateTime, 6));
    ...
    dateTime.isBefore(endDateTime) || isSameDate(dateTime, endDateTime)) {
      ...
      currentViewIndex++;
      dateTime = DateTimeHelperT.getDateTimeValue(addDays(dateTime, 1));
    }

    return -1;
  }

  static AppointmentViewT _createAppointmentView(
      List<AppointmentViewT> appointmentCollection) {
    AppointmentViewT? appointmentView;
    for (int i = 0; i < appointmentCollection.length; i++) {
