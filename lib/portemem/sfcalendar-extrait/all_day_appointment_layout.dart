...
class _AllDayAppointmentLayoutState extends State<AllDayAppointmentLayoutT> {
  ...
  List<AppointmentViewT> _appointmentCollection = <AppointmentViewT>[];
  ...
  @override
  void didUpdateWidget(AllDayAppointmentLayoutT oldWidget) {
    widget.updateCalendarState(_updateCalendarStateDetails);
  ...
  void _updateAppointmentDetails() {
    ...
    _appointmentCollection = <AppointmentViewT>[];
    ...
    if (widget.isExpandable && maxPosition > position && !widget.isExpanding) {
      ...
      for (int i = 0; i < keys.length; i++) {
        ...
        final List<AppointmentViewT> appointmentViews = _indexAppointments[key]!;
        int count = 0;
        ...
        for (final AppointmentViewT view in appointmentViews) {
...
class _AllDayAppointmentRenderWidget extends MultiChildRenderObjectWidget {
  ...
  @override
  _AllDayAppointmentRenderObjectT createRenderObject(BuildContext context) {
    return _AllDayAppointmentRenderObjectT(
        calendar,
        view,
        visibleDates,
        visibleAppointments,
  ...
  void updateRenderObject(
      BuildContext context, _AllDayAppointmentRenderObjectT renderObject) {
    renderObject
...
class _AllDayAppointmentRenderObjectT extends CustomCalendarRenderObjectT {
  _AllDayAppointmentRenderObjectT(
      this.calendar,
      this._view,
      this._visibleDates,
  ...
  /// appointment.
  String _getAllDayAppointmentText(CalendarAppointmentT appointment) {
    if (!CalendarViewHelperT.isDayView(
        view,
        calendar.timeSlotViewSettings.numberOfDaysInView,
        calendar.timeSlotViewSettings.nonWorkingDays,
  ...
  void _drawAllDayAppointmentView(
      PaintingContext context, Offset offset, AppointmentViewT appointmentView) {
  ...
  void _addSelectionForAllDayPanel(Canvas canvas, Size size) {
    final int index = DateTimeHelperT.getIndex(
  ...
  void _addSelectionForAppointment(
      Canvas canvas, AppointmentViewT appointmentView) {
    Decoration? selectionDecoration = calendar.selectionDecoration;
    selectionDecoration ??= BoxDecoration(
      color: Colors.transparent,
      border: Border.all(color: calendarTheme.selectionBorderColor!, width: 2),
  ...
  List<CustomPainterSemantics> _getSemanticsBuilder(Size size) {
    ...
    for (int i = 0; i < appointmentCollection.length; i++) {
      ...
      semanticsBuilder.add(CustomPainterSemantics(
        ...
        properties: SemanticsProperties(
          label:
          CalendarViewHelperT.getAppointmentSemanticsText(view.appointment!),
          textDirection: TextDirection.ltr,
        ),
      ));
    }
