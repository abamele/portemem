...
Widget prepareWidget(dynamic object,
  ...
  return (Text("Bilinmeyen tip: ${object.runtimeType.toString()}"));
}
...
class SearchableDropdownT<T> extends StatefulWidget {
  ...
  factory SearchableDropdownT.single({
    Key? key,
  ...
  /// @param readOnly [bool] whether to let the user choose the value to select or just present the selected value if any.
  /// @param menuBackgroundColor [Color] background color of the menu whether in dialog box or menu mode.
  factory SearchableDropdownT.multiple({
    Key? key,
    required List<DropdownMenuItem<T>> items,
    required Function onChanged,
    ...
    dynamic icon = const Icon(Icons.arrow_drop_down),
    dynamic underline,
    dynamic doneButton = "Kapat",
    dynamic label,
    dynamic closeButton = "Kaydet",
    bool displayClearIcon = true,
    Icon clearIcon = const Icon(Icons.clear),
    Color? iconEnabledColor,
    Color? iconDisabledColor,
  ...
  @override
  _SearchableDropdownTState<T> createState() => new _SearchableDropdownTState();
}
...
class _SearchableDropdownTState<T> extends State<SearchableDropdownT<T>> {
  ...
  @override
  void didUpdateWidget(SearchableDropdownT oldWidget) {
    super.didUpdateWidget(oldWidget as dynamic);
    _updateSelectedIndex();
  }

  Widget get menuWidget {
    return (DropdownDialogT(
      items: widget.items,
...
class DropdownDialogT<T> extends StatefulWidget {
  final List<DropdownMenuItem<T>>? items;
  final Widget? hint;
  final bool isCaseSensitiveSearch;
  final dynamic closeButton;
  ...
  _DropdownDialogTState<T> createState() => new _DropdownDialogTState<T>();
}

class _DropdownDialogTState<T> extends State<DropdownDialogT> {
