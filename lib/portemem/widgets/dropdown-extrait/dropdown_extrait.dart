...
class _DropdownMenuItemButtonState<T> extends State<_DropdownMenuItemButton<T>> {
  ...
  void _handleOnTap() {
    final DropdownMenuItemT<T> dropdownMenuItem = widget.route.items[widget.itemIndex].item!;
...
class _MenuItem<T> extends SingleChildRenderObjectWidget {
  ...
  final DropdownMenuItemT<T>? item;
...
class DropdownMenuItemT<T> extends _DropdownMenuItemContainer {
  /// Creates an item for a dropdown menu.
  ///
  /// The [child] argument is required.
  const DropdownMenuItemT({
    super.key,
    this.onTap,
    this.value,
    this.enabled = true,
...
class DropdownButton<T> extends StatefulWidget {
  ...
  DropdownButton({
    ...
    // DropdownButtonFormField.
      ...
      items.where((DropdownMenuItemT<T> item) {
        return item.value == value;
      }).length == 1,
  "There should be exactly one item with [DropdownButton]'s value: "
      '$value. \n'
      'Either zero or 2 or more [DropdownMenuItemT]s were detected '
      'with the same value',
  ),
  ...
  final List<DropdownMenuItemT<T>>? items;
...
class _DropdownButtonState<T> extends State<DropdownButton<T>> with WidgetsBindingObserver {
  ...
  void _updateSelectedIndex() {
    if (widget.items == null
        ...
        || (widget.value == null &&
            widget.items!
                .where((DropdownMenuItemT<T> item) => item.enabled && item.value == widget.value)
                .isEmpty)) {
      _selectedIndex = null;
      return;
    }

    assert(widget.items!.where((DropdownMenuItemT<T> item) => item.value == widget.value).length == 1);
...
class DropdownButtonFormField<T> extends FormField<T> {
  ...
  DropdownButtonFormField({
    super.key,
    required List<DropdownMenuItemT<T>>? items,
  ...
  "There should be exactly one item with [DropdownButton]'s value: "
      ...
      'with the same value',
        ...
        builder: (FormFieldState<T> field) {
          ...
          final bool showSelectedItem = items != null && items.where((DropdownMenuItemT<T> item) => item.value == state.value).isNotEmpty;
